...
Proof.
  ...
  rewrite <- (sup_le _ _ (j + 1 + i)%nat). simpl.
...
Proof.
  ...
  rewrite <- (sup_le _ _ (n + m*(1+n))%nat).
  simpl.
  rewrite natOrdSize_add.
...
Proof.
  ...
  - rewrite <- lub_le2.
    ...
    { transitivity ((a+b) * ω).
      ...
      apply mul_omega_collapse with n.
      auto. }
    apply mulOrd_monotone2.
    transitivity (1 * ω).
    apply mulOrd_one_l.
