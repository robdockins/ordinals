Require Import Morphisms.
Require Import Coq.Program.Basics.
Require Import NArith.
Require Import List.
Import ListNotations.
Open Scope list.
Require Import Lia.

Unset Printing Records.

From Ordinal Require Import Defs.
From Ordinal Require Import Operators.
From Ordinal Require Import Arith.
From Ordinal Require Import Cantor.
From Ordinal Require Import Fixpoints.
From Ordinal Require Import Reflection.
From Ordinal Require Import VeblenDefs.
From Ordinal Require Import VeblenCon.


Open Scope ord_scope.


Inductive VForm : Set :=
| Z : VForm
| V : VForm -> VForm -> VForm
| W : VForm -> VForm -> VForm.

Fixpoint VF_denote (x:VForm) : Ord :=
  match x with
  | Z => 0
  | V a x => veblen (addOrd 1) (VF_denote a) (VF_denote x)
  | W a x => veblen (expOrd ω) (VF_denote a) (VF_denote x)
  end.

Canonical Structure VF := ord VForm VF_denote.

(*
Fixpoint VF_normal (x:VForm) : Prop :=
  match x with
  | Z => True
  | V a x => VF_normal a /\ VF_normal x /\
             match x with
             | Z => True
             | V b y => VF_denote a >= VF_denote b
             end
  end.
*)

Lemma VF_complete (x:VForm) : complete (VF_denote x).
Proof.
  induction x.
  - apply zero_complete.
  - simpl.
    apply veblen_complete; auto.
    apply onePlus_normal; auto.
    intros; apply addOrd_complete; auto.
    apply succ_complete; apply zero_complete.
  - simpl.
    apply veblen_complete; auto.
    apply powOmega_normal.
    intros; apply expOrd_complete; auto.
    apply (index_lt _ 0%nat).
    apply omega_complete.
Qed.

Lemma omega_gt0 : 0 < ω.
Proof. apply (index_lt _ 0%nat). Qed.

Local Hint Unfold powOmega : core.
Local Hint Resolve VF_complete veblen_complete
        onePlus_normal powOmega_normal expOrd_complete addOrd_complete
        omega_complete succ_complete zero_complete
        normal_monotone normal_complete omega_gt0 omega_gt1 : core.

Lemma V_collapse :
  forall a b x,
    VF_denote a < VF_denote b ->
    VF_denote (V a (V b x)) ≈ VF_denote (V b x).
Proof.
  intros. simpl.
  apply veblen_fixpoints; auto.
Qed.

Lemma W_collapse :
  forall a b x,
    VF_denote a < VF_denote b ->
    VF_denote (W a (W b x)) ≈ VF_denote (W b x).
Proof.
  intros. simpl.
  apply veblen_fixpoints; auto.
Qed.

Lemma WZ_collapse :
  forall a, VF_denote (W Z a) ≈ VF_denote (V a Z).
Proof.
  simpl; intros.
  rewrite veblen_zero.
  rewrite veblen_onePlus; auto.
  rewrite addOrd_zero_r.
  reflexivity.
Qed.

Lemma VW_collapse : forall a b c,
  0 < VF_denote b ->
  VF_denote a < VF_denote (W b c) ->
  VF_denote (V a (W b c)) ≈ VF_denote (W b c).
Proof.
  simpl VF_denote.
  intros.
  split.
  2: { apply veblen_inflationary; auto. }
  rewrite veblen_onePlus; auto.
  apply additively_closed_collapse; auto.
  red; intros.
  rewrite <- (veblen_fixpoints _ powOmega_normal 0 (VF_denote b) (VF_denote c)); auto.
  rewrite <- (veblen_fixpoints _ powOmega_normal 0 (VF_denote b) (VF_denote c)) in H1, H2; auto.
  rewrite veblen_zero in *.
  apply expOmega_additively_closed; auto.
  rewrite <- (veblen_fixpoints _ powOmega_normal 0 (VF_denote b) (VF_denote c)) in *; auto.
  rewrite veblen_zero in *.
  apply expOrd_increasing; auto.
  rewrite <- (veblen_fixpoints _ powOmega_normal 0 (VF_denote b) (VF_denote c)) in *; auto.
  rewrite veblen_zero in *.
  auto.
Qed.    


Lemma VW_collapse2 : forall b c,
  0 < VF_denote b ->
  VF_denote (V (W b c) Z) ≈ VF_denote (W b c).
Proof.
  intros. simpl.
  split.
  - rewrite <- (veblen_fixpoints _ (powOmega_normal) 0) at 2; auto.
    rewrite veblen_zero.
    rewrite veblen_onePlus; auto.
    rewrite addOrd_zero_r.
    reflexivity.
  - apply (normal_inflationary (fun x => veblen (addOrd 1) x 0)); auto.
    apply veblen_first_normal; auto.
Qed.

Lemma veblen_additively_closed a b :
  complete a -> complete b ->
  additively_closed (veblen (expOrd ω) a b).
Proof.
  intros. red. intros x y Hx Hy.
  destruct (complete_zeroDec a) as [Ha|Ha]; auto.
  - assert (veblen (expOrd ω) a b ≈ expOrd ω b).
    { split.
      transitivity (veblen (expOrd ω) 0 b).
      apply veblen_monotone_first; auto.
      rewrite veblen_zero. reflexivity.
      rewrite veblen_unroll.
      apply lub_le1. }
    rewrite H1 in Hx, Hy.
    rewrite H1.
    apply expOmega_additively_closed; auto.
  - assert (veblen (expOrd ω) a b ≈ expOrd ω (veblen (expOrd ω) a b)).
    { rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.  reflexivity. }
    rewrite H1 in Hx, Hy.
    rewrite H1.
    apply expOmega_additively_closed; auto.
Qed.

Lemma Γ_additively_closed a :
  complete a -> additively_closed (Γ a).
Proof.
  red; intros.
  rewrite Γ_fixpoints; auto.
  rewrite Γ_fixpoints in H0, H1; auto.
  apply veblen_additively_closed; auto.
  apply normal_complete; auto.
  apply Γ_normal.
Qed.  


Lemma VF_below_Γ₀ : forall x, VF_denote x < Γ 0.
Proof.
  induction x; simpl VF_denote.
  - apply normal_nonzero. apply Γ_normal.
  - rewrite Γ_fixpoints; auto.
    rewrite <- (veblen_fixpoints _ powOmega_normal (VF_denote x1) (Γ 0) 0); auto.
    apply ord_le_lt_trans with
        (veblen powOmega (VF_denote x1) (VF_denote x2)).
    apply veblen_monotone_func; auto.
    intros. apply onePlus_least_normal; auto.
    apply veblen_increasing; auto.
    apply veblen_complete; auto.
    apply normal_complete; auto.
    apply Γ_normal.
    apply ord_lt_le_trans with (Γ 0); auto.
    apply (normal_inflationary (fun x => veblen powOmega x 0)).
    apply veblen_first_normal; auto.
    apply normal_complete; auto.
    apply Γ_normal.
    apply normal_complete; auto.
    apply Γ_normal.
  - rewrite Γ_fixpoints; auto.
    rewrite <- (veblen_fixpoints _ powOmega_normal (VF_denote x1) (Γ 0) 0); auto.
    apply veblen_increasing; auto.
    apply veblen_complete; auto.
    apply normal_complete; auto.
    apply Γ_normal.
    apply ord_lt_le_trans with (Γ 0); auto.
    apply (normal_inflationary (fun x => veblen powOmega x 0)).
    apply veblen_first_normal; auto.
    apply normal_complete; auto.
    apply Γ_normal.
    apply normal_complete; auto.
    apply Γ_normal.
Qed.


Lemma VF_limit : limitOrdinal VF.
Proof.
  hnf. split.
  exact (inhabits Z).
  red; simpl; intros.
  exists (V a (V Z Z)).
  simpl.
  rewrite veblen_onePlus; auto.
  apply ord_le_lt_trans with (expOrd ω (VF_denote a)).
  apply normal_inflationary; auto.
  rewrite <- addOrd_zero_r at 1.
  apply addOrd_increasing.
  apply veblen_nonzero; auto.
Qed.

Lemma VF_veblen_closed : veblen powOmega VF 0 ≤ VF.
Proof.
  transitivity (veblen powOmega (boundedSup VF (fun x => x)) 0).
  { apply veblen_monotone_first.
    intros; apply expOrd_monotone; auto.
    apply (ord_isLimit VF). apply VF_limit. }
  unfold boundedSup. unfold VF at 1.
  transitivity (supOrd (fun x => veblen powOmega (VF_denote x) 0)).
  { apply veblen_continuous_first; auto. exact Z. }
  apply sup_least. intro x.
  transitivity (VF_denote (W x Z)).
  simpl; reflexivity.
  apply index_le.
Qed.

Theorem VF_Γ₀ : VF ≈ Γ 0.
Proof.
  split.
  - rewrite ord_le_unfold. intro x.
    apply VF_below_Γ₀.
  - unfold Γ.
    rewrite enum_fixpoints_zero; auto.
    unfold fixOrd. apply sup_least.
    intro i; induction i.
    + simpl; auto with ord.
    + simpl.
      transitivity (veblen powOmega VF 0).
      apply veblen_monotone_first; auto.
      apply VF_veblen_closed.
    + apply veblen_first_normal; auto.
Qed.


Fixpoint VF_compare (x:VForm) : VForm -> ordering :=
  fix inner (y:VForm) : ordering :=
    match x, y with
    | Z, Z     => EQ
    | Z, V _ _ => LT
    | Z, W _ _ => LT
    | V _ _, Z => GT
    | W _ _, Z => GT

    | V a x', V b y' =>
      match VF_compare a b with 
      | LT => VF_compare x' y
      | EQ => VF_compare x' y'
      | GT => inner y'        
      end

    | W a x', W b y' =>
      match VF_compare a b with 
      | LT => VF_compare x' y
      | EQ => VF_compare x' y'
      | GT => inner y'        
      end

    | V a x', W b y' =>
      match b with
      | Z => 
        match VF_compare a y' with
        | LT => VF_compare x' (V y' Z)
        | EQ => VF_compare x' Z
        | GT => GT
        end
      | _ =>
        match VF_compare a (W b y') with
        | LT => VF_compare x' (W b y')
        | EQ => match x' with
                | Z => EQ
                | _ => GT
                end
        | GT => GT
        end
      end

    | W a x', V b y' =>
      match a with
      | Z =>
        match VF_compare x' b with
        | LT => LT
        | EQ => match y' with
                | Z => EQ
                | _ => LT
                end
        | GT => inner y'
        end
      | _ =>
        match inner b with
        | LT => LT
        | EQ => match y' with
                | Z => EQ
                | _ => LT
                end
        | GT => inner y'
        end
      end
    end.


Definition ordering_correct o a b :=
  match o with
  | LT => a < b
  | EQ => a ≈ b
  | GT => a > b
  end.

Lemma veblen_compare_same_level f (Hf:normal_function f) :
  forall oab oxy oxVby oVaxy a b x y,
    complete a ->
    complete b ->
    complete x ->
    complete y ->
    ordering_correct oab a b ->
    ordering_correct oxy x y ->
    ordering_correct oxVby x (veblen f b y) ->
    ordering_correct oVaxy (veblen f a x) y ->
    ordering_correct
      (match oab with
      | LT => oxVby
      | EQ => oxy
      | GT => oVaxy
      end) (veblen f a x) (veblen f b y) .
Proof.
  do 8 intro. intros Ha Hb Hx Hy Hoab Hoxy HoxVby HVaxy.
  destruct oab; simpl in *.
  - destruct oxVby; simpl in *.
    + apply ord_lt_le_trans with (veblen f a (veblen f b y)).
      apply veblen_increasing; auto.
      apply veblen_fixpoints; auto.
    + transitivity (veblen f a (veblen f b y)).
      { split; (apply veblen_monotone; [ intros; apply normal_monotone; auto with ord | apply HoxVby ]). }
      apply veblen_fixpoints; auto.
    + apply ord_lt_le_trans with x; auto.
      apply veblen_inflationary; auto.
  - destruct oxy; simpl in *.
    + apply ord_lt_le_trans with (veblen f a y).
      apply veblen_increasing; auto.
      apply veblen_monotone_first; auto.
      apply Hoab.
    + transitivity (veblen f a y).
      split; apply veblen_monotone; auto; apply Hoxy.
      split; apply veblen_monotone_first; auto; apply Hoab.
    + apply ord_le_lt_trans with (veblen f a y).
      apply veblen_monotone_first; auto. apply Hoab.
      apply veblen_increasing; auto.
  - destruct oVaxy; simpl in *.
    + apply ord_lt_le_trans with y; auto.
      apply veblen_inflationary; auto.
    + symmetry.
      transitivity (veblen f b (veblen f a x)).
      split; apply veblen_monotone; auto; apply HVaxy.
      apply veblen_fixpoints; auto.
    + apply ord_lt_le_trans with (veblen f b (veblen f a x)).
      apply veblen_increasing; auto.
      apply veblen_fixpoints; auto.
Qed.      


Lemma VF_compare_correct : forall x y,
    ordering_correct (VF_compare x y) (VF_denote x) (VF_denote y).
Proof.
  unfold ordering_correct.
  induction x as [|a IHa x IHx|a IHa x IHx].
  (* x => Z case *)
  - destruct y as [|b y|b y]; simpl; auto with ord.
    apply veblen_nonzero; auto.
    apply veblen_nonzero; auto.
  (* x => V a x case *)
  - induction y as [|b IHb y IHy|b IHb y IHy]; simpl.
    (* y => Z case *)
    + apply veblen_nonzero; auto.
    (* y => V b y case *)
    + apply (veblen_compare_same_level); auto.
      apply (IHa b). apply (IHx y). apply IHx.

    (* y => W b y case *)
    + assert (b = Z \/ b <> Z).
      { destruct b; auto; right; discriminate. }
      destruct H.
      * subst b. simpl.
        generalize (veblen_compare_same_level
                      (addOrd 1) onePlus_normal
                      (VF_compare a y) (VF_compare x Z)
                      (VF_compare x (V y Z)) GT
                      (VF_denote a) (VF_denote y) (VF_denote x)  0
                      (VF_complete _)
                      (VF_complete _)
                      (VF_complete _)
                      zero_complete
                      (IHa y) (IHx Z) (IHx (V y Z)) (veblen_nonzero _ onePlus_normal _ _)
                   ).
        { destruct (VF_compare a y).
          - destruct (VF_compare x (V y Z)); simpl;
              rewrite veblen_zero;
              repeat (rewrite veblen_onePlus; auto);
              rewrite addOrd_zero_r; auto.
          - destruct (VF_compare x Z); simpl;
              rewrite veblen_zero;
              repeat (rewrite veblen_onePlus; auto);
              rewrite addOrd_zero_r; auto.
          - simpl;
              rewrite veblen_zero;
              repeat (rewrite veblen_onePlus; auto);
              rewrite addOrd_zero_r; auto. }

      * cut (match match VF_compare a (W b y) with
                   | LT => VF_compare x (W b y)
                   | EQ => match x with
                           | Z => EQ
                           | _ => GT
                           end
                   | GT => GT
                   end
             with
             | LT =>
               veblen (addOrd 1) (VF_denote a) (VF_denote x) <
               veblen (expOrd ω) (VF_denote b) (VF_denote y)
             | EQ =>
               veblen (addOrd 1) (VF_denote a) (VF_denote x)
                      ≈ veblen (expOrd ω) (VF_denote b) (VF_denote y)
             | GT =>
               veblen (expOrd ω) (VF_denote b) (VF_denote y) <
               veblen (addOrd 1) (VF_denote a) (VF_denote x)
             end).
        { destruct b; auto. elim H; auto. }
        generalize (IHa (W b y)).
        destruct (VF_compare a (W b y)); intros Hasub.
        ** generalize (IHx (W b y)).
           destruct (VF_compare x (W b y)); intros Hxsub.
           *** rewrite veblen_onePlus; auto.
               apply veblen_additively_closed; auto.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
               rewrite veblen_zero.
               apply expOrd_increasing; auto.
               destruct b; simpl.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.

           *** rewrite veblen_onePlus; auto.
               rewrite Hxsub.
               simpl.
               split.
               apply additively_closed_collapse.
               apply veblen_additively_closed; auto.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
               rewrite veblen_zero.
               apply expOrd_increasing; auto.
               destruct b; simpl.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
               apply addOrd_le2.
           *** rewrite veblen_onePlus; auto.
               rewrite <- addOrd_le2. apply Hxsub.
        ** destruct x; simpl.
           *** rewrite veblen_onePlus; auto.
               rewrite addOrd_zero_r.
               rewrite Hasub.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
               rewrite veblen_zero.
               simpl. reflexivity.
               destruct b; simpl.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.               
           *** rewrite veblen_onePlus; auto.
               rewrite Hasub.
               simpl.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               rewrite <- addOrd_zero_r.
               apply addOrd_increasing.
               apply veblen_nonzero; auto.
               destruct b; simpl.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.               
           *** rewrite veblen_onePlus; auto.
               rewrite Hasub.
               simpl.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               rewrite <- addOrd_zero_r.
               apply addOrd_increasing.
               apply veblen_nonzero; auto.
               destruct b; simpl.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
        ** rewrite veblen_onePlus; auto.
           rewrite <- addOrd_le1.
           rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
           rewrite veblen_zero.
           apply expOrd_increasing; auto.
           destruct b; simpl.
           elim H; auto.
           apply veblen_nonzero; auto.
           apply veblen_nonzero; auto.

  (* x => W a x case *)
  - induction y as [|b IHb y IHy|b IHb y IHy]; simpl.
    (* y => Z case *)
    + apply veblen_nonzero; auto.
    (* y => V b y case *)
    + assert (a = Z \/ a <> Z).
      { destruct a; auto; right; discriminate. }
      destruct H.
      * subst a.
        assert (Hy' : 
                  ordering_correct match y with
                    | Z => EQ
                    | _ => LT
                    end 0 (VF_denote y)).
        { destruct y; simpl; auto with ord.
          apply veblen_nonzero; auto.
          apply veblen_nonzero; auto. }
        assert (HWZxy :
                  (ordering_correct (VF_compare (W Z x) y)
                                    (veblen (addOrd 1) (VF_denote x) 0) (VF_denote y))).
        { destruct (VF_compare (W Z x) y); simpl in IHy; simpl.
          - rewrite veblen_zero in IHy.
            rewrite veblen_onePlus; auto.
            rewrite addOrd_zero_r. auto.
          - rewrite veblen_zero in IHy.
            rewrite veblen_onePlus; auto.
            rewrite addOrd_zero_r. auto.
          - rewrite veblen_zero in IHy.
            rewrite veblen_onePlus; auto.
            rewrite addOrd_zero_r. auto. }

        generalize (veblen_compare_same_level
                      (addOrd 1) onePlus_normal
                      (VF_compare x b) (match y with Z => EQ | _ => LT end)
                      LT
                      (VF_compare (W Z x) y) 
                      (VF_denote x) (VF_denote b) 0 (VF_denote y)
                      (VF_complete _)
                      (VF_complete _)
                      zero_complete
                      (VF_complete _)
                      (IHx b) Hy' (veblen_nonzero _ onePlus_normal _ _) HWZxy).
        { destruct (VF_compare x b); simpl.
            + rewrite veblen_zero.
              rewrite veblen_onePlus; auto.
              rewrite addOrd_zero_r. auto.
            + destruct (match y with
                   | Z => EQ
                   | _ => LT
                   end); simpl;
              rewrite veblen_zero;
              repeat (rewrite veblen_onePlus; auto);
              rewrite addOrd_zero_r; auto.
            + destruct ((fix inner (y0 : VForm) : ordering :=
        match y0 with
        | Z => GT
        | V b0 y' =>
            match VF_compare x b0 with
            | LT => LT
            | EQ => match y' with
                    | Z => EQ
                    | _ => LT
                    end
            | GT => inner y'
            end
        | W b0 y' =>
            match
              (fix inner0 (y1 : VForm) : ordering :=
                 match y1 with
                 | Z => EQ
                 | _ => LT
                 end) b0
            with
            | LT => VF_compare x y0
            | EQ => VF_compare x y'
            | GT => inner y'
            end
        end) y); simpl;
              rewrite veblen_zero;
              repeat (rewrite veblen_onePlus; auto);
              rewrite addOrd_zero_r; auto. }

      (* a <> Z case *)
      * cut (match match VF_compare (W a x) b with
                   | LT => LT
                   | EQ => match y with
                           | Z => EQ
                           | _ => LT
                           end
                   | GT => VF_compare (W a x) y
                   end with
                 | LT =>
      veblen (expOrd ω) (VF_denote a) (VF_denote x) <
      veblen (addOrd 1) (VF_denote b) (VF_denote y)
  | EQ =>
      veblen (expOrd ω) (VF_denote a) (VF_denote x)
      ≈ veblen (addOrd 1) (VF_denote b) (VF_denote y)
  | GT =>
      veblen (addOrd 1) (VF_denote b) (VF_denote y) <
      veblen (expOrd ω) (VF_denote a) (VF_denote x) end).
        { destruct a; auto. elim H; auto. }
        destruct (VF_compare (W a x) b).
        ** rewrite veblen_onePlus; auto.
           rewrite <- addOrd_le1.
           simpl in IHb.
           rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
           rewrite veblen_zero.
           apply expOrd_increasing; auto.
           destruct a; simpl; auto.
           elim H; auto.
           apply veblen_nonzero; auto.
           apply veblen_nonzero; auto.

        ** destruct y; simpl.
           *** simpl in IHb.
               rewrite veblen_onePlus; auto.
               rewrite addOrd_zero_r.
               rewrite <- IHb.               
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               reflexivity.
               destruct a; simpl; auto.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
           *** rewrite veblen_onePlus; auto.
               rewrite <- IHb.
               simpl.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               rewrite <- addOrd_zero_r.
               apply addOrd_increasing; auto.
               apply veblen_nonzero; auto.
               destruct a; simpl; auto.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
           *** rewrite veblen_onePlus; auto.
               rewrite <- IHb.
               simpl.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               rewrite <- addOrd_zero_r.
               apply addOrd_increasing; auto.
               apply veblen_nonzero; auto.
               destruct a; simpl; auto.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
        ** destruct (VF_compare (W a x) y).
           *** rewrite veblen_onePlus; auto.
               rewrite <- addOrd_le2.
               apply IHy.
           *** rewrite veblen_onePlus; auto.
               rewrite <- IHy.
               split.
               apply addOrd_le2.
               apply additively_closed_collapse; auto.
               apply veblen_additively_closed; auto.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               apply expOrd_increasing; auto.
               destruct a; simpl; auto.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.
           *** rewrite veblen_onePlus; auto.
               simpl in *.
               apply veblen_additively_closed; auto.
               rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
               rewrite veblen_zero.
               apply expOrd_increasing; auto.
               destruct a; simpl; auto.
               elim H; auto.
               apply veblen_nonzero; auto.
               apply veblen_nonzero; auto.

    (* y => W b y case *)
    + apply (veblen_compare_same_level); auto.
      apply (IHa b). apply (IHx y). apply IHx.
Qed.

Global Opaque VF_compare.

Definition Vnorm (a:VForm) (b:VForm) : VForm :=
  match b with
  | Z     => V a Z
  | V x y => match VF_compare a x with
             | LT => b
             | _  => V a b
             end
  | W Z y => match VF_compare a y with
             | LT => b
             | _  => V a b
             end
  | W _ _ => match VF_compare a b with
             | LT => b
             | _  => V a b
             end
  end.


Definition Wnorm (a:VForm) (b:VForm) : VForm :=
  match b with
  | Z => W a Z
  | W x y => match VF_compare a x with
             | LT => b
             | _  => W a b
             end
  | _ => W a b
  end.

Fixpoint VF_hnf (x:VForm) : VForm :=
  match x with
  | Z => Z
  | V a b => Vnorm a (VF_hnf b)
  | W Z b => V b Z
  | W a b => Wnorm a (VF_hnf b)
  end.


Fixpoint VF_isHnf (x:VForm) : Prop :=
  match x with
  | Z => True
  | V a b => VF_isHnf b /\
             match b with
             | Z => True
             | V x y => VF_denote a >= VF_denote x
             | W _ _ => VF_denote a >= VF_denote b
             end
  | W a b => a <> Z /\
             VF_isHnf b /\
             match b with
             | W x y => VF_denote a >= VF_denote x
             | _ => True
             end
  end.

Lemma VF_hnf_isHnf : forall x, VF_isHnf (VF_hnf x).
Proof.
  induction x; simpl; intuition.
  unfold Vnorm.
  - destruct (VF_hnf x2); simpl in *; intuition.
    generalize (VF_compare_correct x1 v1).
    destruct (VF_compare x1 v1); simpl in *; intuition.
    apply H1.
    destruct v1.
    + generalize (VF_compare_correct x1 v2).
      destruct (VF_compare x1 v2); simpl in *; intuition.
    + generalize (VF_compare_correct x1 (W (V v1_1 v1_2) v2)).
      destruct (VF_compare x1 (W (V v1_1 v1_2) v2)); simpl in *; intuition.
      apply H0.
    + generalize (VF_compare_correct x1 (W (W v1_1 v1_2) v2)).
      destruct (VF_compare x1 (W (W v1_1 v1_2) v2)); simpl in *; intuition.
      apply H0.
  - destruct x1; simpl; auto.
    + unfold Wnorm. simpl in *.
      destruct (VF_hnf x2); simpl in *; intuition; try discriminate.
      generalize (VF_compare_correct (V x1_1 x1_2) v1).
      destruct (VF_compare (V x1_1 x1_2) v1); simpl; intuition; try discriminate.
      apply H0.
    + unfold Wnorm.
      destruct (VF_hnf x2); simpl in *; intuition; try discriminate.
      generalize (VF_compare_correct (W x1_1 x1_2) v1).
      destruct (VF_compare (W x1_1 x1_2) v1); simpl; intuition; try discriminate.
      apply H0.
Qed.

Lemma Vnorm_equal : forall a b , VF_denote (Vnorm a b) ≈ (VF_denote (V a b)).
Proof.
  intros; unfold Vnorm; simpl.
  destruct b; simpl; auto with ord.
  - generalize (VF_compare_correct a b1).
    destruct (VF_compare a b1); simpl; auto with ord.
    intros; symmetry; apply veblen_fixpoints; auto.
  - destruct b1.
    + generalize (VF_compare_correct a b2).
      destruct (VF_compare a b2); simpl; intuition.
      rewrite veblen_onePlus; auto.
      rewrite veblen_zero.
      symmetry.
      split.
      apply additively_closed_collapse.
      apply expOmega_additively_closed; auto.
      apply expOrd_increasing; auto.
      apply addOrd_le2.
    + generalize (VF_compare_correct a (W (V b1_1 b1_2) b2)).
      destruct (VF_compare a (W (V b1_1 b1_2) b2)); auto with ord.
      intros.
      symmetry.
      apply VW_collapse; auto.
      simpl. apply veblen_nonzero; auto.
    + generalize (VF_compare_correct a (W (W b1_1 b1_2) b2)).
      destruct (VF_compare a (W (W b1_1 b1_2) b2)); auto with ord.
      intros.
      symmetry.
      apply VW_collapse; auto.
      simpl. apply veblen_nonzero; auto.
Qed.


Lemma Wnorm_equal : forall a b , VF_denote (Wnorm a b) ≈ (VF_denote (W a b)).
Proof.
  intros; unfold Wnorm; simpl.
  destruct b; simpl; auto with ord.
  generalize (VF_compare_correct a b1).
  destruct (VF_compare a b1); simpl; auto with ord.
  intros; symmetry; apply veblen_fixpoints; auto.
Qed.  

Lemma VF_hnf_equal : forall x, VF_denote (VF_hnf x) ≈ VF_denote x.
Proof.
  induction x; simpl; auto with ord.
  rewrite Vnorm_equal. simpl.
  split; apply veblen_monotone; auto; apply IHx2.
  destruct x1; simpl; auto with ord.
  rewrite veblen_zero.
  rewrite veblen_onePlus; auto.
  rewrite addOrd_zero_r.
  reflexivity.
  rewrite Wnorm_equal. simpl.
  split; apply veblen_monotone; auto; apply IHx2.
  rewrite Wnorm_equal. simpl.
  split; apply veblen_monotone; auto; apply IHx2.
Qed.  


Lemma VF_isHnf_dominates : forall (b a:VF),
    VF_isHnf (V a b) -> exists n:ω, expOrd ω a * n ≥ b.
Proof.
  simpl. induction b; simpl; intuition.
  - exists 0%nat. auto with ord.
  - destruct (IHb2 a) as [n Hn]; intuition.
    destruct b2; simpl in *; intuition.
    rewrite H2; auto.
    rewrite H2; auto.
    exists (n+1)%nat.
    rewrite natOrdSize_add.
    rewrite ordDistrib_left.
    simpl.
    rewrite mulOrd_one_r.
    rewrite veblen_onePlus; auto.
    apply addOrd_monotone; auto.
  - destruct (IHb2 a) as [n Hn]; intuition.
    destruct b2; simpl in *; intuition.
    rewrite <- H1.
    transitivity (veblen (addOrd 1) (VF_denote b2_1) 0).
    apply (normal_inflationary (fun x => veblen (addOrd 1) x 0)); auto.
    apply veblen_first_normal; auto.
    transitivity (veblen (addOrd 1) (VF_denote b2_1) (VF_denote b2_2)).
    apply veblen_monotone; auto with ord.
    apply normal_inflationary; auto.
    apply veblen_normal; auto.
    rewrite <- H1.
    apply normal_inflationary; auto.
    apply veblen_normal; auto.
    exists 1%nat. simpl.
    rewrite mulOrd_one_r.
    rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
    rewrite veblen_zero.
    apply expOrd_monotone; auto.
    destruct b1; simpl.
    elim H; auto.
    apply veblen_nonzero; auto.
    apply veblen_nonzero; auto.
Qed.

Definition VF_isZero x : { x = Z } + { 0 < VF_denote x }.
Proof.
  destruct x.
  - left. apply eq_refl.
  - right. simpl. apply veblen_nonzero; auto.
  - right. simpl. apply veblen_nonzero; auto.
Qed.


Fixpoint VF_add (x y:VForm) : VForm :=
  match x with
  | Z     => y
  | V a b => V a (VF_add b y)
  | W a b => if VF_isZero a then V b y else V x y
  end.

Lemma VF_add_correct x y : VF_denote (VF_add x y) ≈ VF_denote x + VF_denote y.
Proof.
  induction x; simpl.
  - rewrite addOrd_zero_l. reflexivity.
  - rewrite veblen_onePlus; auto.
    rewrite veblen_onePlus; auto.
    rewrite <- addOrd_assoc.
    rewrite IHx2; auto with ord.
  - destruct (VF_isZero x1).
    + subst x1. simpl.
      rewrite veblen_zero.
      rewrite veblen_onePlus; auto.
      reflexivity.
    + simpl.
      rewrite veblen_onePlus; auto.
      apply addOrd_eq_mor; auto with ord.
      symmetry.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.
      reflexivity.
Qed.      

Definition VF_one := V Z Z.
Definition VF_succ x := VF_add x VF_one.

Lemma VF_one_correct : VF_denote VF_one ≈ 1.
Proof.
  simpl. rewrite veblen_onePlus; auto.
  rewrite addOrd_zero_r.
  rewrite expOrd_zero.
  reflexivity.
Qed.

Lemma VF_succ_correct x : VF_denote (VF_succ x) ≈ succOrd (VF_denote x).
Proof.
  unfold VF_succ.
  rewrite VF_add_correct.
  rewrite VF_one_correct.
  rewrite addOrd_succ.
  rewrite addOrd_zero_r.
  reflexivity.
Qed.

(* x * ωᵉ *)
Definition VF_mul_single (x:VForm) (e:VForm) : VForm :=
  if VF_isZero e then x else
    match x with
    (* x * ω⁰ = x *)
    | Z => Z
    (* (ωᵃ + q) * ωᵉ = ω^(a + e) otherwise *)
    | V a _ => V (VF_add a e) Z
    (* (W a b) * ωᵉ  = ω^(W a b) * ωᵉ = ω^(W a b + e) for a > 0 *)
    | W _ _ => V (VF_add x e) Z
    end.

Lemma VF_mul_single_correct : forall x e,
    VF_isHnf x ->
    VF_denote (VF_mul_single x e) ≈ VF_denote x * expOrd ω (VF_denote e).
Proof.
  unfold VF_mul_single. intros.
  destruct (VF_isZero e).
  - subst e. simpl.
    rewrite expOrd_zero.
    rewrite mulOrd_one_r.
    reflexivity.
  - destruct x; simpl.
    + symmetry. apply mulOrd_zero_l. 
    + rewrite veblen_onePlus at 1; auto.
      rewrite addOrd_zero_r.
      rewrite VF_add_correct.
      rewrite expOrd_add.
      rewrite veblen_onePlus at 1; auto.
      destruct (VF_isHnf_dominates x2 x1) as [n Hn]; auto.
      split.
      apply mulOrd_monotone1.
      apply addOrd_le1.
      apply expOrd_omega_collapse with n; auto.
    + simpl.
      destruct (VF_isZero x1).
      { simpl in *; intuition. }
      simpl.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_zero_r.
      rewrite veblen_onePlus; auto.
      rewrite expOrd_add.
      apply mulOrd_eq_mor; auto with ord.
      symmetry.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.
      apply expOrd_eq_mor; auto with ord.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.
      reflexivity.
Qed.        


Definition VF_mul x : VForm -> VForm :=
  let x' := VF_hnf x in
  fix loop (y:VForm) : VForm :=
  match y with
  | Z => Z
  | V b y' => VF_add (VF_mul_single x' b) (loop y')
  | W b y' => if VF_isZero b then VF_mul_single x' y' else VF_mul_single x' y
  end.


Lemma VF_mul_correct x y : VF_denote (VF_mul x y) ≈ VF_denote x * VF_denote y.
Proof.
  induction y; simpl.
  - rewrite mulOrd_zero_r; auto with ord.
  - rewrite VF_add_correct.
    rewrite VF_mul_single_correct.
    rewrite veblen_onePlus; auto.
    rewrite ordDistrib_left.
    apply addOrd_eq_mor; auto.
    apply mulOrd_eq_mor; auto with ord.
    apply VF_hnf_equal.
    apply VF_hnf_isHnf.
  - destruct (VF_isZero y1).
    + subst y1.
      simpl. rewrite veblen_zero.
      rewrite VF_mul_single_correct.
      apply mulOrd_eq_mor; auto with ord.
      apply VF_hnf_equal.
      apply VF_hnf_isHnf.
    + rewrite VF_mul_single_correct.
      apply mulOrd_eq_mor; auto with ord.
      apply VF_hnf_equal.
      simpl.
      symmetry.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.
      reflexivity.
      apply VF_hnf_isHnf.
Qed.

Definition VF_isFinite x : { n:nat | VF_denote x ≈ sz n } + { 1 + VF_denote x ≈ VF_denote x }.
Proof.
  induction x; simpl.
  - left. exists 0%nat. simpl; reflexivity.
  - destruct (VF_isZero x1).
    + subst x1. simpl.
      destruct IHx2 as [ [n Hn] | Hinf ].
      * left. exists (n+1)%nat.
        rewrite veblen_onePlus; auto.
        rewrite expOrd_zero.
        rewrite natOrdSize_add.
        apply addOrd_eq_mor; simpl; auto with ord.
      * right.
        rewrite veblen_onePlus; auto.
        rewrite expOrd_zero.
        apply addOrd_eq_mor; auto with ord.
    + right.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_assoc.
      apply addOrd_eq_mor; auto with ord.
      split.
      apply additively_closed_collapse.
      apply expOmega_additively_closed; auto.
      apply ord_lt_le_trans with (expOrd ω 1).
      rewrite expOrd_one'; auto.
      apply expOrd_monotone.
      apply succ_least; auto.
      apply addOrd_le2.
  - destruct (VF_isZero x1).
    + subst x1.
      destruct (VF_isZero x2).
      * subst x2. simpl.
        left. exists 1%nat.
        rewrite veblen_zero.
        rewrite expOrd_zero.
        auto with ord.
      * right.
        split.
        apply additively_closed_collapse.
        apply veblen_additively_closed; auto.
        simpl. rewrite veblen_zero.
        apply ord_lt_le_trans with (expOrd ω 1).
        rewrite expOrd_one'; auto.
        apply expOrd_monotone.
        apply succ_least; auto.
        apply addOrd_le2.
    + right.
      split.
      apply additively_closed_collapse.
      apply veblen_additively_closed; auto.
      apply ord_lt_le_trans with (expOrd ω 1).
      rewrite expOrd_one'; auto.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0); auto.
      rewrite veblen_zero.
      apply expOrd_monotone.
      apply succ_least; auto.
      apply veblen_nonzero; auto.
      apply addOrd_le2.
Qed.      

Fixpoint VF_nat (n:nat) :=
  match n with
  | O => Z
  | S n' => V Z (VF_nat n')
  end.

Lemma VF_nat_correct : forall n,
    VF_denote (VF_nat n) ≈ sz n.
Proof.
  induction n; simpl; auto with ord.
  rewrite veblen_zero.
  transitivity (sz (n+1)%nat).
  rewrite natOrdSize_add.
  apply addOrd_eq_mor; auto with ord.
  replace (n+1)%nat with (1+n)%nat by lia.
  simpl. auto with ord.
Qed.

Definition VF_exp_single (x:VForm) (e:VForm) :=
  match VF_isFinite x with
  | inleft (exist _ n _ ) =>
    match n with
    | 0 => VF_one
    | 1 => VF_one
    | _ => match VF_isFinite e with
           | inleft (exist _ en _) =>
             match en with
             | 0 => x
             | S m => V (V (VF_nat m) Z) Z
             end
           | inright He => V (V e Z) Z 
           end
    end

  | inright Hx =>
    if VF_isZero e then x else
      match x with
      | Z => VF_one
      | V a _ => V (VF_mul a (V e Z)) Z 
      | W _ _ => V (VF_mul x (V e Z)) Z
      end
  end.

Local Opaque VF_mul.

Lemma VF_exp_single_correct x e :
  VF_isHnf x ->
  VF_denote (VF_exp_single x e) ≈ expOrd (VF_denote x) (expOrd ω (VF_denote e)).
Proof.
  unfold VF_exp_single. intros.
  destruct (VF_isFinite x) as [[n Hn]|Hinf]; simpl.
  - destruct n.
    { simpl. rewrite veblen_zero. rewrite addOrd_zero_r.
      rewrite Hn.
      simpl.
      split.
      apply succ_least. apply expOrd_nonzero.
      rewrite expOrd_unfold.
      apply lub_least; auto with ord.
      apply sup_least; intro.
      rewrite mulOrd_zero_r. auto with ord. }
    destruct n.  
    { simpl. rewrite veblen_zero.
      rewrite addOrd_zero_r.
      rewrite Hn.
      symmetry. apply expOrd_one_base. }
    destruct (VF_isFinite e) as [[m Hm]|Hinfe].
    + destruct m. 
      { simpl. 
        rewrite Hm.
        simpl.
        rewrite expOrd_zero.
        rewrite expOrd_one'; auto with ord.
        rewrite Hn.
        simpl.
        apply succ_trans; auto with ord. }
      simpl.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_zero_r.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_zero_r.
      rewrite Hm.
      symmetry.
      rewrite Hn.
      transitivity (expOrd (sz (S (S n))) (expOrd ω (1+sz m))).
      apply expOrd_eq_mor; auto with ord.
      apply expOrd_eq_mor; auto with ord.
      transitivity (sz (m+1)%nat).
      replace (m+1)%nat with (1+m)%nat by lia.
      simpl. auto with ord.
      rewrite natOrdSize_add; auto with ord.
      rewrite expNatToOmegaPow.
      rewrite VF_nat_correct. reflexivity.
      simpl.
      apply succ_trans.
      apply succ_least.
      apply succ_trans.
      auto with ord.
    + rewrite <- Hinfe.
      rewrite Hn.
      rewrite expNatToOmegaPow.
      simpl.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_zero_r.
      rewrite veblen_onePlus; auto.
      rewrite addOrd_zero_r.
      reflexivity.
      simpl.
      apply succ_trans.
      apply succ_least.
      apply succ_trans.
      auto with ord.
  - destruct (VF_isZero e).
    + subst e. simpl.
      rewrite expOrd_zero.
      rewrite expOrd_one'; auto with ord.
      rewrite <- Hinf.
      rewrite <- addOrd_le1.
      apply succ_lt.
    + destruct x; simpl.
      * rewrite veblen_zero.
        rewrite addOrd_zero_r.
        rewrite expOrd_unfold.
        split.
        apply lub_le1.
        apply lub_least; auto with ord.
        apply sup_least; intro.
        rewrite mulOrd_zero_r. auto with ord.
      * rewrite veblen_onePlus; auto.
        rewrite addOrd_zero_r.
        rewrite VF_mul_correct.
        rewrite expOrd_mul.
        simpl.
        rewrite veblen_onePlus; auto.
        rewrite addOrd_zero_r.
        split.
        apply expOrd_monotone_base.
        rewrite veblen_onePlus; auto.
        apply addOrd_le1.
        rewrite veblen_onePlus; auto.
        destruct (VF_isHnf_dominates x2 x1) as [n Hn]; auto.
        rewrite expToOmega_collapse_tower with n; auto with ord.
        transitivity (expOrd ω 1).
        { rewrite expOrd_one'.
          transitivity (natOrdSize (1+n)).
          rewrite natOrdSize_add. reflexivity.
          apply index_le.
          auto. }
        apply expOrd_monotone.
        apply succ_least.
        destruct (VF_isZero x1); auto.
        subst x1.
        simpl in Hinf.
        rewrite veblen_zero in Hinf.
        assert (forall n:ω, sz n + (1 + VF_denote x2) ≈ sz n + VF_denote x2 -> VF_isHnf (V Z x2) -> False).
        { clear. induction x2; simpl; intros.
          - rewrite addOrd_zero_r in H.
            rewrite addOrd_zero_r in H.
            apply (ord_lt_irreflexive (natOrdSize n)).
            rewrite <- H at 2.
            rewrite addOrd_succ.
            rewrite addOrd_zero_r.
            apply succ_lt.
          - simpl in H.
            rewrite addOrd_assoc in H.
            intuition.
            apply (IHx2_2 (S n)); simpl; intuition.
            transitivity 
              (natOrdSize n + 1 + veblen (addOrd 1) (VF_denote x2_1) (VF_denote x2_2)).
            apply addOrd_eq_mor.
            rewrite addOrd_succ.
            rewrite addOrd_zero_r.
            reflexivity.
            rewrite veblen_onePlus; auto.
            apply addOrd_eq_mor; auto with ord.
            split. apply succ_least. apply expOrd_nonzero.
            rewrite H2.
            rewrite expOrd_zero. auto with ord.
            rewrite H.
            rewrite veblen_onePlus; auto.
            rewrite addOrd_assoc.
            apply addOrd_eq_mor.
            transitivity (natOrdSize n + 1).
            apply addOrd_eq_mor; auto  with ord.
            split.
            rewrite H2.
            rewrite expOrd_zero; auto with ord.
            apply succ_least. apply expOrd_nonzero.
            rewrite addOrd_succ. rewrite addOrd_zero_r.
            reflexivity.
            reflexivity.
            destruct x2_2; auto.
            rewrite H3; auto.
            rewrite H3; auto.

          - intuition.
            elim (ord_lt_irreflexive 0).
            rewrite <- H2 at 2.
            apply veblen_nonzero; auto. }
        elim (H0 1%nat); auto.
      * rewrite veblen_onePlus; auto.
        rewrite addOrd_zero_r.
        rewrite VF_mul_correct.
        rewrite expOrd_mul.
        simpl.
        rewrite veblen_onePlus; auto.
        rewrite addOrd_zero_r.        
        apply expOrd_eq_mor; auto with ord.
        symmetry.
        rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto with ord.
        rewrite veblen_zero.
        reflexivity.
        simpl in  H.
        destruct x1; intuition; simpl.
        apply veblen_nonzero; auto.
        apply veblen_nonzero; auto.
Qed.        


Definition VF_exp (x:VForm) : VForm -> VForm :=
  let x' := VF_hnf x in
  fix loop (y:VForm) : VForm :=
  match y with
  | Z      => VF_one
  | V b y' => VF_mul (VF_exp_single x' b) (loop y')
  | W b y' => if VF_isZero b then VF_exp_single x' y' else VF_exp_single x' y
  end.

Lemma VF_exp_correct : forall x y,
  VF_denote (VF_exp x y) ≈ expOrd (VF_denote x) (VF_denote y).
Proof.
  intro x. induction y; simpl.
  - rewrite veblen_zero.
    rewrite expOrd_zero.
    rewrite addOrd_zero_r. reflexivity.
  - rewrite VF_mul_correct.
    rewrite VF_exp_single_correct; auto.
    rewrite veblen_onePlus; auto.
    rewrite expOrd_add.
    apply mulOrd_eq_mor; auto.
    apply expOrd_eq_mor; auto with ord.
    apply VF_hnf_equal.
    apply VF_hnf_isHnf.
  - destruct (VF_isZero y1).
    + subst y1. simpl.
      rewrite veblen_zero.
      rewrite VF_exp_single_correct.
      apply expOrd_eq_mor; auto with ord.
      apply VF_hnf_equal.
      apply VF_hnf_isHnf.
    + rewrite VF_exp_single_correct.
      apply expOrd_eq_mor; auto.
      apply VF_hnf_equal.
      symmetry.
      rewrite <- (veblen_fixpoints _ powOmega_normal 0) at 1; auto.
      rewrite veblen_zero.
      reflexivity.
      apply VF_hnf_isHnf.
Qed.


Require Import ClassicalFacts.
From Ordinal Require Import Classical.


Lemma veblen_decompose (EM:excluded_middle) f (Hf:normal_function f) :
  forall x
    (Hlim : limitOrdinal x),
    x < veblen f x 0 ->
    f x ≤ x ->
    exists a b, x ≈ veblen f a b /\ 0 < a /\ a < x /\ b < x.
Proof.
  induction x as [x Hx] using ordinal_induction; intros.
  set (P a := a > 0 /\ exists b, b < x /\ x <= veblen f a b).
  destruct (classical.ord_well_ordered EM P x) as [a [??]]; auto.
  { red. split. rewrite <- H0. apply normal_nonzero; auto.
    exists 0. split; auto with ord. rewrite <- H0. apply normal_nonzero; auto. }

  hnf in H1.
  destruct H1 as [Ha0 [b0 Hb0]].
  set (Q b := b < x /\ x <= veblen f a b).
  destruct (classical.ord_well_ordered EM Q b0) as [b [??]]; auto.
  unfold P in *.
  unfold Q in *.

  assert (Hle : veblen f a b ≤ x).
  { destruct (classical.order_total EM (veblen f a b) x); auto.
    exfalso.
    rewrite veblen_unroll in H4.
    apply lub_lt in H4.
    destruct H4.
    - apply (ord_lt_irreflexive x).
      apply ord_lt_le_trans with (f b); auto.
      transitivity (f x); auto.
      intuition.

    - case_eq a; intros A g Hg.
      rewrite Hg in H4. simpl in H4.
      apply sup_lt in H4.
      destruct H4 as [i H4].
      
      unfold fixOrd in H4.
      apply sup_lt in H4.
      destruct H4 as [n ?].
      apply (ord_lt_irreflexive x).
      eapply ord_lt_le_trans. apply H4.
      clear H4.
      induction n; simpl.
      * rewrite ord_le_unfold; simpl; intro q.
        destruct (classical.order_total EM x (veblen f (ord A g) (b q))); auto. exfalso.
        rewrite <- Hg in H4.
        assert (b <= b q).
        { apply H3; split; auto.
          transitivity b; auto with ord.
          intuition. }
        elim (ord_lt_irreflexive b).
        rewrite H5 at 1.
        apply index_lt.

      * transitivity (veblen f (g i) x).
        apply veblen_monotone; auto.
        clear n IHn.
        destruct (classical.order_total EM (veblen f (g i) x) x); auto. exfalso.
        assert (a <= g i).
        { apply H2.
          split.
          - destruct (classical.order_total EM (g i) 0); auto.
            elim (ord_lt_irreflexive x).
            apply ord_lt_le_trans with (veblen f (g i) x); auto.
            transitivity (veblen f 0 x).
            apply veblen_monotone_first; auto.
            rewrite veblen_zero; auto.
          - assert (x ≈ supOrd (fun i => x i)).
            { destruct x as [X h].
              apply ascending_sup_lim.
              destruct Hlim; auto.
            }
            assert (veblen f (g i) x ≤ veblen f (g i) (supOrd (fun i : x => x i))).
            { apply veblen_monotone; auto. apply H5. }
            assert (veblen f (g i) (supOrd (fun j : x => x j)) <=
                    supOrd (fun j => veblen f (g i) (x j))).
            { assert (x > 0).
              { rewrite <- H0. apply normal_nonzero; auto. }
              rewrite ord_lt_unfold in H7. destruct H7; auto.
              apply veblen_continuous; auto.
              apply classical.ord_complete; auto.
              apply classical.ord_directed; auto.
              intros; apply classical.ord_complete; auto. }
            rewrite H6 in H4. rewrite H7 in H4.
            apply sup_lt in H4.
            destruct H4 as [j Hj].
            exists (x j); split; auto with ord. }
        apply (ord_lt_irreflexive a).
        rewrite H5 at 1.
        rewrite Hg.
        apply (index_lt (ord A g) i). }

  assert (Hax : a < x).
  { destruct (classical.order_total EM x a); auto. exfalso.
    elim (ord_lt_irreflexive x). 
    apply ord_lt_le_trans with (veblen f x 0); auto.
    transitivity (veblen f a 0).
    apply veblen_monotone_first; auto.
    rewrite <- Hle.
    apply veblen_monotone; auto with ord. }

  exists a, b; intuition.
  split; auto.
Qed.


(* True, but we don't need it? 
Lemma limitOrdinal_intro : forall x,
    x > 0 ->
    (forall i, i < x -> exists j, i < j /\ j < x) ->
    limitOrdinal x.
Proof.
  destruct x as [X f]; intros.
  simpl; split.
  - rewrite ord_lt_unfold in H. destruct H as [i _].
    exact (inhabits i).
  - hnf. intros.
    destruct (H0 (f a)) as [b [??]].
    apply (index_lt (ord X f) a).
    rewrite ord_lt_unfold in H2.
    destruct H2 as [k ?].
    exists k.
    apply ord_lt_le_trans with b; auto.
Qed.

Lemma expOrd_limit : forall x, x > 0 -> limitOrdinal (expOrd ω x).
Proof.
  destruct x as [X f]. simpl; intros.
  apply limitOrdinal_intro.
  apply expOrd_nonzero.
  intros.
  rewrite expOrd_unfold in H0.
  apply lub_lt in H0. destruct H0.
  - exists 1; split; auto.
    rewrite expOrd_unfold.
    rewrite <- lub_le2.
    rewrite ord_lt_unfold in H.
    destruct H as [x ?].
    rewrite <- (sup_le _ _ x).
    simpl.
    rewrite mulOrd_unfold.
    simpl.
    rewrite <- (sup_le _ _ 1%nat).
    simpl.
    rewrite mulOrd_one_r.
    apply ord_lt_le_trans with (1+1).
    rewrite addOrd_succ.
    rewrite addOrd_zero_r.
    apply succ_lt.
    apply addOrd_monotone; apply succ_least; apply expOrd_nonzero.
  - apply sup_lt in H0.
    destruct H0 as [x Hx].
    rewrite mulOrd_unfold in Hx.
    apply sup_lt in Hx.
    destruct Hx as [n Hn]. simpl in Hn.
    exists (expOrd ω (f x) * sz (S n)).
    split; simpl; auto.
    rewrite mulOrd_succ. auto.
    rewrite (expOrd_unfold _ (ord X f)).
    rewrite <- lub_le2.
    rewrite <- (sup_le _ _ x).
    rewrite (mulOrd_unfold _ ω).
    rewrite mulOrd_succ.
    rewrite <- (sup_le _ _ (S n)).
    simpl.
    rewrite mulOrd_succ.
    rewrite <- addOrd_assoc.
    apply addOrd_increasing.
    rewrite <- addOrd_zero_r at 1.
    apply addOrd_increasing.
    apply expOrd_nonzero.
Qed.
*)

Lemma onePlus_succ x : 1 + x ≤ succOrd x.
Proof.
  induction x as [X f Hx].
  rewrite addOrd_unfold. simpl.
  apply lub_least. apply succOrd_le_mor. auto with ord.
  apply sup_least; intro i.
  apply succOrd_le_mor.
  rewrite Hx.
  apply succ_least. apply (index_lt (ord X f) i).
Qed.  

Lemma limit_onePlus x : limitOrdinal x -> 1 + x ≤ x.
Proof.
  destruct x as [X f]; simpl; intros [??].
  rewrite addOrd_unfold.
  apply lub_least.
  apply succ_least.
  destruct H as [x]. rewrite ord_lt_unfold. exists x. auto with ord.
  apply sup_least; intro x.
  apply succ_least. simpl.
  hnf in H0.
  destruct (H0 x) as [y Hy].
  apply ord_le_lt_trans with (f y).
  transitivity (succOrd (f x)).
  apply onePlus_succ.
  apply succ_least. auto.
  apply (index_lt (ord X f) y).
Qed.


Lemma VW_has_enough_notations (EM:excluded_middle) :
  forall x, x < Γ 0 -> exists (v:VF), v ≈ x.
Proof.
  induction x as [x Hx] using ordinal_induction. intro H.
  destruct (classical.ordinal_discriminate EM x) as [Hzero|[Hsucc|Hlimit]].
  - exists Z. simpl.
    apply ord_isZero in Hzero. symmetry; auto.
  - apply ord_isSucc in Hsucc.
    destruct Hsucc as [o Ho].
    destruct (Hx o) as [vo Hvo].
    rewrite Ho. apply succ_lt.
    transitivity x; auto.
    rewrite Ho. apply succ_lt.
    exists (VF_succ vo).
    rewrite Ho. rewrite <- Hvo.
    apply VF_succ_correct.
  - assert (1 + x <= x).
    { apply limit_onePlus; auto. }
    destruct (classical.order_total EM (veblen (addOrd 1) x 0) x).
    * rewrite veblen_onePlus in H1; auto.
      2:{ apply classical.ord_complete; auto. }
      rewrite addOrd_zero_r in H1.
      destruct (veblen_decompose EM powOmega powOmega_normal x) as [a [b [?[_[??]]]]]; auto.
      
      { destruct (classical.order_total EM (veblen powOmega x 0) x); auto.
        elim (ord_lt_irreflexive (Γ 0)).
        apply ord_le_lt_trans with x; auto.
        unfold Γ.
        simpl.
        unfold fixOrd. apply sup_least; intro i.
        induction i; simpl.
        rewrite ord_le_unfold; intros [].
        rewrite <- H2.
        apply veblen_monotone_first; auto. }

      destruct (Hx a) as [va ?]; auto.
      transitivity x; auto.
      destruct (Hx b) as [vb ?]; auto.
      transitivity x; auto.
      exists (W va vb).
      simpl. rewrite H2.
      transitivity (veblen (expOrd ω) a (VF_denote vb)).
      split; apply veblen_monotone_first; auto with ord; apply H5.
      split; apply veblen_monotone; auto; apply H6.
      
    * destruct (veblen_decompose EM (addOrd 1) (onePlus_normal) x) as [a [b [?[_[??]]]]]; auto.
      destruct (Hx a) as [va ?]; auto.
      transitivity x; auto.
      destruct (Hx b) as [vb ?]; auto.
      transitivity x; auto.
      exists (V va vb).
      rewrite H2. simpl.
      transitivity (veblen (addOrd 1) a (VF_denote vb)).
      split; apply veblen_monotone_first; auto; apply H5.
      split; apply veblen_monotone; auto; apply H6.
Qed.
